...
from common.models.sys_user_info import SysUserInfo
'''
post/index 列表
post/info 详情
post/set 添加|编辑
...
def get():
    #var_a = request.args.get( "a","i love wuqingvika" )
    ##变种
    req = request.values
    var_a = req['a'] if "a" in req else "i love wuqingvika"
    return "request:%s,params:%s,var_a:%s"%(request.method,request.args,var_a )

@index_page.route("/post",methods = [ "POST" ])
def post():
    #三元表达式
    #var_a = request.form['a'] if "a" in request.form else 'i love wuqingvika'
    #普通容易看懂的
    # var_a = ""
...
def template():
    ...
    ##
    context = { "name" : name }
    context['user'] = { "nickname":"wuqingvika","qq":"xxxxx","home_page":"wuqingvika.github.io" }
